...
/*if(i==0)
//    {
		mexPrintf("\n\tVector to be rotated is :\t%f \t%lf \t%lf \t%lf",qv[0],qv[1],qv[2],qv[3]);
		mexPrintf("\n\tq*v is :\t%f \t%lf \t%lf \t%lf",qt[0],qt[1],qt[2],qt[3]);
		mexPrintf("\n\tRotated output is :q*v*qconj :\t%f \t%lf \t%lf \t%lf",qf[0],qf[1],qf[2],qf[3]);
//	}*/	
  }
  return;
...
void f_construct_quaternion(FVec w, float theta, FQuat q)
{
 ...
 // mexEvalString("drawnow;");
  ...
  q[2] = st2*w[1];
  q[3] = st2*w[2];  
  //mexPrintf("\n\tQ is :\t%lf %lf %lf %lf",q[0],q[1],q[2],q[3]);
